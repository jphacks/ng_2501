...
class GeneratedScene(Scene):
    def construct(self):
        ...
        y_label = right_axes.get_y_axis_label(
            MathTex("\\mathrm{Area}\\,(\\mathrm{cm}^2)").set_color(WHITE), edge=LEFT, direction=LEFT, buff=0.6
        )
        ...
        P_label = always_redraw(
            ...
            .scale(0.7)
            .set_color(WHITE)
            .next_to(left_axes.c2p(*P_of_t(t.get_value())), UR, buff=0.08)
        )
        ...
        area_label = VGroup(MathTex("\\mathrm{Area}(\\triangle ADP)=").scale(0.9).set_color(WHITE), area_value).arrange(
            RIGHT, buff=0.08
        )
        ...
        full_curve = right_axes.plot(
            lambda tau: area_ADP(tau), x_range=[0, T_END], use_smoothing=False, color=GRAY_B, stroke_opacity=0.25
        )

        prog_curve = always_redraw(
