...
class GeneratedScene(Scene):
    def construct(self):
        ...
        case_box  = RoundedRectangle(corner_radius=0.15, width=5.6, height=1.0, color=GRAY_E)\
                        .set_fill(color=GRAY, opacity=0.20)
        ...
        left_top = VGroup(title, left_formula_grp, param_row, D_label)\
        ...
        axes = Axes(
            ...
            x_length=AX_X_LEN, y_length=AX_Y_LEN,
        ...
        parabola_graph = always_redraw(
            lambda: axes.plot(lambda x: f(x), x_range=[X_MIN, X_MAX], color=YELLOW)
        )
            ...
            if D > EPS:
                ...
                for xv, name in ((x1, "x_1"), (x2, "x_2")):
                    if X_MIN <= xv <= X_MAX:
                        ...
                        dash = DashedLine(axes.c2p(xv, 0), axes.c2p(xv, f(xv)), color=GREEN,
        ...
        def update_D_and_c(_m=None):
            ...
            c_num.set_value(c)
            D_num.set_value(D)
            if D > EPS:   D_num.set_color(GREEN)
            elif D < -EPS: D_num.set_color(RED)
            else:         D_num.set_color(ORANGE)
