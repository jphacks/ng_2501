...
class ManimAnimationService:
    def __init__(self):
        ...
        if prompts_path.exists():
            ...
            self.graph_logger.warning("prompts.toml が見つかりません。デフォルトのプロンプトを使用します。")
    ...
    def _generate_initial_script(self, state: ManimGraphState):
        ...
        return {"current_script": script, "current_retry": 0}
    ...
    def _refine_script_on_error(self, state: ManimGraphState):
        ...
        fixed_script = chain.invoke(
            {
                "concept_summary": state["user_request"],
