...
class GeneratedScene(Scene):
    def construct(self):
        ...
        def P(x, y): return plane.c2p(x, y)

        def A(): return np.array([ax.get_value(), ay.get_value(), 0.0])
        def B(): return np.array([bx.get_value(), by.get_value(), 0.0])

        arrowA = always_redraw(lambda:
            Arrow(P(0,0), P(*A()[:2]), buff=0,
                  max_tip_length_to_length_ratio=0.12, stroke_width=7, color=BLUE)
        )
        arrowB = always_redraw(lambda:
            Arrow(P(0,0), P(*B()[:2]), buff=0,
                  max_tip_length_to_length_ratio=0.12, stroke_width=7, color=GREEN)
        )
        arrowA_at_B = always_redraw(lambda:
            Arrow(P(*B()[:2]), P(*(B()[:2] + A()[:2])), buff=0,
                  max_tip_length_to_length_ratio=0.12, stroke_width=4, color=BLUE_D, stroke_opacity=0.9)
        )
        arrowB_at_A = always_redraw(lambda:
            Arrow(P(*A()[:2]), P(*(A()[:2] + B()[:2])), buff=0,
                  max_tip_length_to_length_ratio=0.12, stroke_width=4, color=GREEN_D, stroke_opacity=0.9)
        )
        arrowSum = always_redraw(lambda:
            Arrow(P(0,0), P(*(A()[:2] + B()[:2])), buff=0,
                  max_tip_length_to_length_ratio=0.12, stroke_width=7, color=YELLOW)
        )
        ...
        guide_B_from_A = always_redraw(lambda:
            dashed_line(P(*A()[:2]), P(*(A()[:2] + B()[:2])), color=GREEN, width=3, dashed_ratio=0.55, density=8, z=6)
        )
        guide_A_from_B = always_redraw(lambda:
            dashed_line(P(*B()[:2]), P(*(B()[:2] + A()[:2])), color=BLUE, width=3, dashed_ratio=0.55, density=8, z=6)
        )
        para_outline = always_redraw(lambda:
            dashed_polygon([P(0,0), P(*A()[:2]), P(*(A()[:2] + B()[:2])), P(*B()[:2])],
                           color=WHITE, width=2, dashed_ratio=0.55, n=60, z=4)
        )
        para_fill = always_redraw(lambda:
            Polygon(P(0,0), P(*A()[:2]), P(*(A()[:2] + B()[:2])), P(*B()[:2]),
                    stroke_color=WHITE, stroke_opacity=0.85, stroke_width=2,
                    fill_color=YELLOW, fill_opacity=0.12
            ).set_z_index(1)
        )

        labelA = always_redraw(lambda: Text("A", font_size=36, color=BLUE)
                               .next_to(arrowA.get_end(), UR, buff=0.18))
        labelB = always_redraw(lambda: Text("B", font_size=36, color=GREEN)
                               .next_to(arrowB.get_end(), UR, buff=0.18))
        labelS = always_redraw(lambda: Text("A+B", font_size=36, color=YELLOW)
                               .next_to(arrowSum.get_end(), UL, buff=0.18))

        def spacer(w=0.14):
            return Rectangle(width=w, height=0.001).set_stroke(opacity=0).set_fill(opacity=0)

        def vector_row_stable(name, color, tx: ValueTracker, ty: ValueTracker,
                              name_size=26, num_size=36, buff=0.16, close_gap=0.06):
